...
# TARS_FLEET_HEALTH — fleet-compatible endpoints
def _tars_fleet_health():
    try:
        cfg = globals().get("CFG") or {}
        if not cfg:
            import json as _j, os as _o
            cp = _o.path.join(_o.path.dirname(_o.path.abspath(__file__)), "config.json")
            cfg = _j.load(open(cp)) if _o.path.exists(cp) else {}
        m = (cfg.get("model") or {})
        return {"status":"ok","agent":"TARS","role":"Builder","runtime":"tars-python","model": m.get("model","llama-3.3-70b-versatile")}
...
def anthropic_chat(system, messages, max_tokens=600):
    ...
    if use_or:
        ...
        # Pauli fix: when base_url is set (Groq/OpenRouter/etc), model is already the full slug
        model_slug = model if ("/" in str(model) or base) else f"anthropic/{model}"
        body = json.dumps({
            "model": model_slug,
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        # TARS_FLEET_HEALTH_ROUTE
        p = self.path.split('?')[0]
        if p in ('/health', '/fleet/health'):
            import json as _json
            body = _json.dumps(_tars_fleet_health()).encode()
    ...
    def do_POST(self):
        # TARS_FLEET_MISSION_ROUTE
        p = self.path.split('?')[0]
        if p in ('/mission', '/fleet/mission'):
            import json as _json
            try:
                n = int(self.headers.get('Content-Length') or 0)
                raw = self.rfile.read(n) if n else b'{}'
                payload = _json.loads(raw or b'{}')
                mission = payload.get('mission') or payload.get('message') or payload.get('question') or ''
                # Prefer existing chat/brain path if available
                ans = None
                try:
                    if 'call_model' in globals():
                        ans, err = call_model([{'role':'user','content': mission}], 'You are TARS, Builder for The Pauli Effect. Be concise.', 300)
                        if err: ans = err
                    elif 'think' in globals():
                        ans = think(mission)
                except Exception as e:
                    ans = 'TARS received mission: ' + mission + ' (' + str(e) + ')'
                if not ans:
                    ans = 'TARS accepted mission. Plan: ' + (mission[:200] if mission else 'none')
                body = _json.dumps({'accepted': True, 'response': ans, 'model': ((globals().get('CFG') or {}).get('model') or {}).get('model','llama-3.3-70b-versatile'), 'runtime':'tars-python'}).encode()
                self.send_response(200); self.send_header('Content-Type','application/json'); self.send_header('Content-Length', str(len(body))); self.end_headers(); self.wfile.write(body); return
            except Exception as e:
